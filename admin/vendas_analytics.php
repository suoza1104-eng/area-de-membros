...
<div class="bi">
  ...
  <div class="two-col">
    ...
    <section class="section-card"><div class="section-head"><div><h2>Conversao por turma</h2><p>Gasto de trafego somado entre a primeira e a ultima entrada de aluno na turma.</p></div></div><div class="table-wrap"><table class="bi-table cohort-table"><thead><tr><th>Turma</th><th>Entradas</th><th>Alunos</th><th>Tráfego</th><th>CPL</th><th>Vendas</th><th>ROAS</th><th>Conversao</th><th>Bruto</th><th>Liquido produtor</th></tr></thead><tbody><?php foreach($cohorts as $r):?><tr><td><strong><?=va_h($r['turma'])?></strong></td><td><?=!empty($r['entry_start'])&& !empty($r['entry_end']) ? va_h(date('d/m/y',strtotime((string)$r['entry_start'])).' a '.date('d/m/y',strtotime((string)$r['entry_end']))) : '-'?></td><td><?=va_num($r['leads'])?></td><td><?=va_money($r['traffic_cost'] ?? 0)?></td><td><?=va_money($r['cpl'] ?? 0)?></td><td><?=va_num($r['sales'])?></td><td><?=va_num($r['roas'] ?? 0,2)?></td><td><?=va_pct($r['conversion'])?></td><td><?=va_money($r['gross'])?></td><td><?=va_money($r['producer'])?></td></tr><?php endforeach;?><?php if(!$cohorts):?><tr><td colspan="10" class="empty">Sem turmas atribuidas no periodo.</td></tr><?php endif;?></tbody></table></div></section>
  </div>
