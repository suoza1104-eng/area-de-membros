...
function applyDraft(d){nameInput.value=d.name||nameInput.value;subject.value=d.subject||subject.value;preheader.value=d.preheader||preheader.value;v.innerHTML=safeHtml(d.html||v.innerHTML);if(!code.hidden)code.value=v.innerHTML;sync();runChecks()}
async function runAiDraft(action){aiCreateStatus.className='ee-status';aiCreateStatus.textContent='Gerando rascunho...';aiGenerate.disabled=aiAdjust.disabled=true;try{const d=await post(action,{prompt:aiPrompt.value,inspiration_id:aiInspiration.value});applyDraft(d.draft);aiCreateStatus.textContent=(d.draft.notes||'Rascunho aplicado no editor. Revise, peça ajuste ou aprove.')}catch(e){aiCreateStatus.className='ee-status error';aiCreateStatus.textContent=e.message}finally{aiGenerate.disabled=aiAdjust.disabled=false}}
aiGenerate.onclick=()=>runAiDraft('ai_generate');aiAdjust.onclick=()=>runAiDraft('ai_adjust');aiApprove.onclick=()=>{aiCreateModal.classList.remove('open');runChecks();open('tipsModal')};
