...
try {
    $paymentProductRows = $pdo->query("
        ...
        UNION
        SELECT DISTINCT TRIM(product_code) v FROM student_payment_events WHERE product_code IS NOT NULL AND TRIM(product_code)<>''
        UNION
        SELECT DISTINCT TRIM(product_name) v FROM payment_sales WHERE product_name IS NOT NULL AND TRIM(product_name)<>''
        UNION
        SELECT DISTINCT TRIM(external_product_id) v FROM payment_sales WHERE external_product_id IS NOT NULL AND TRIM(external_product_id)<>''
        ORDER BY v LIMIT 500
    ")->fetchAll(PDO::FETCH_COLUMN) ?: [];
    $paymentProductOptions = array_values(array_unique(array_filter(array_map(static fn($v) => trim((string)$v), $paymentProductRows), static fn($v) => $v !== '')));
} catch (Throwable $e) {}
...
function paymentProductOptionsHtml(value){const current=String(value||''),list=paymentProducts.concat(current&&!paymentProducts.includes(current)?[current]:[]);return `<input id="iPaymentProduct" list="paymentProductOptions" value="${esc(current)}" placeholder="Todos os cursos/produtos"><datalist id="paymentProductOptions">${list.map(x=>`<option value="${esc(x)}"></option>`).join('')}</datalist>`}
