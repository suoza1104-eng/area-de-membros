...
$templates = $pdo->query("SELECT v.id,t.name,v.version_number,v.subject,(v.id=t.current_version_id) AS is_current FROM email_template_versions v JOIN email_templates t ON t.id=v.template_id WHERE t.status='active' ORDER BY t.name ASC, v.version_number DESC")->fetchAll(PDO::FETCH_ASSOC) ?: [];
...
$paymentGatewayOptions = ['hotmart' => 'Hotmart', 'dom' => 'DOM Pagamentos', 'pagarme' => 'Pagar.me', 'firepay' => 'Firepay'];
try {
    $paymentGatewayRows = $pdo->query("
        SELECT DISTINCT provider v FROM payment_sales WHERE provider IS NOT NULL AND TRIM(provider)<>''
        UNION
        SELECT DISTINCT payment_gateway v FROM payment_sales WHERE payment_gateway IS NOT NULL AND TRIM(payment_gateway)<>''
        ORDER BY v LIMIT 200
    ")->fetchAll(PDO::FETCH_COLUMN) ?: [];
    foreach ($paymentGatewayRows as $paymentGatewayRow) {
        $gatewayValue = strtolower(trim((string)$paymentGatewayRow));
        if ($gatewayValue === '') continue;
        $gatewayLabel = match ($gatewayValue) {
            'hotmart' => 'Hotmart',
            'dom' => 'DOM Pagamentos',
            'pagarme' => 'Pagar.me',
            'firepay' => 'Firepay',
            default => ucfirst($gatewayValue),
        };
        $paymentGatewayOptions[$gatewayValue] = $gatewayLabel;
    }
} catch (Throwable $e) {}
...
.afe-email-create{display:grid;gap:6px;padding:10px;border:1px solid rgba(56,189,248,.28);border-radius:10px;background:rgba(14,165,233,.08)}.afe-email-create .btn{justify-content:center}.afe-email-create small{color:#93c5fd;font-size:10px;line-height:1.4}
...
const automationFlowId=<?=(int)$flow['id']?>;
const paymentGateways=<?=json_encode($paymentGatewayOptions,JSON_UNESCAPED_UNICODE|JSON_HEX_TAG)?>;
...
{label:'Pagamentos',items:[{code:'payment_gateway',label:'Ultimo gateway de pagamento',tag:'Pagamento',desc:'Compara o gateway da compra mais recente do aluno.',value:'gateway'}]},
...
function templateOptions(value, fallbackLabel=''){let found=false;let html='<option value="">Selecione</option>';(templates||[]).forEach(t=>{const isSel=String(value)===String(t.id);if(isSel)found=true;html+=`<option value="${t.id}" ${isSel?'selected':''}>${esc(t.name)} · v${t.version_number}${t.is_current?' (Atual)':''} · ${esc(t.subject)}</option>`});if(value&&!found){html+=`<option value="${esc(value)}" selected>${esc(fallbackLabel||('Modelo #'+value))}</option>`}return html;}
function pct(a,b){return b>0?(100*a/b).toFixed(1).replace('.',',')+'%':'0,0%'}
function abVariantHtml(v,i){const s=abStats[String(v.templateVersionId)]||{},sent=+s.sent||0,delivered=+s.delivered||0,opened=+s.opened||0,clicked=+s.clicked||0,bounced=+s.bounced||0;return `<div class="afe-rule"><div class="afe-rule-head"><strong>Variante ${esc(v.label||String.fromCharCode(65+i))}</strong><button type="button" class="afe-rule-remove" data-ab-remove="${i}">Excluir</button></div><input data-ab="${i}" data-k="label" value="${esc(v.label||'')}" placeholder="Nome da variante"><input type="number" min="1" max="100" data-ab="${i}" data-k="weight" value="${esc(v.weight||0)}" placeholder="Peso %"><select data-ab="${i}" data-k="templateVersionId">${templateOptions(v.templateVersionId, v.templateLabel)}</select><div class="afe-copy">Envios: ${sent} · Entrega ${pct(delivered,sent)} · Abertura ${pct(opened,sent)} · Clique ${pct(clicked,sent)} · Bounce ${pct(bounced,sent)}</div></div>`}
function emailConfigHtml(c){c.variants=Array.isArray(c.variants)&&c.variants.length?c.variants:[{id:uid('var'),label:'A',weight:50,templateVersionId:c.templateVersionId||0,templateLabel:c.templateLabel||''},{id:uid('var'),label:'B',weight:50,templateVersionId:0,templateLabel:''}];const createBox=`<div class="afe-email-create"><button type="button" class="btn btn-primary btn-sm" id="createEmailTemplate">+ Criar novo e-mail</button><small>Abre o editor completo com IA. Ao salvar, este bloco seleciona o novo modelo automaticamente.</small></div>`;return field('Modelo publicado',`<select id="iTemplate" ${c.abEnabled?'disabled':''}>${templateOptions(c.templateVersionId, c.templateLabel)}</select>`)+createBox+`<label class="afe-check"><input id="iAbEnabled" type="checkbox" ${c.abEnabled?'checked':''}> Ativar teste A/B/n neste e-mail</label>`+(c.abEnabled?`<div class="afe-config-box"><div class="afe-field"><label>Variantes A/B/n</label><div class="afe-copy">O peso define a proporção dos disparos. As métricas abaixo usam os envios deste fluxo para cada modelo.</div><div class="afe-fields">${c.variants.map(abVariantHtml).join('')}</div><button type="button" class="btn btn-ghost btn-xs" id="addVariant">+ Inserir variante</button></div></div>`:'')}
...
function paymentGatewayOptions(value){return `<option value="">Selecione o gateway</option>${Object.entries(paymentGateways||{}).map(([k,label])=>`<option value="${esc(k)}" ${String(value)===String(k)?'selected':''}>${esc(label)}</option>`).join('')}`}
...
function defaultOperator(field){return ['course_progress_pct','lessons_completed_count','engagement_count'].includes(field)?'gte':(field==='payment_gateway'?'equals':'has')}
function operatorOptions(field,value){const numeric=['course_progress_pct','lessons_completed_count','engagement_count'].includes(field),gateway=field==='payment_gateway',base=gateway?{equals:'é igual',not_equals:'é diferente'}:(numeric?{gte:'maior ou igual',lte:'menor ou igual',equals:'é igual',not_equals:'é diferente'}:{has:'ocorreu / possui',not_has:'não ocorreu / não possui',equals:'é igual',not_equals:'é diferente',gte:'maior ou igual',lte:'menor ou igual'});return opts(base,value||defaultOperator(field))}
...
document.addEventListener('click',e=>{const btn=e.target.closest('#createEmailTemplate');if(!btn)return;e.preventDefault();const n=graph.nodes.find(x=>x.id===selected);if(!n||n.type!=='email')return;const url='email_editor.php?automation_return=1&flow_id='+encodeURIComponent(automationFlowId)+'&node_id='+encodeURIComponent(n.id);window.open(url,'_blank')});
window.addEventListener('message',e=>{if(e.origin!==location.origin)return;const d=e.data||{};if(d.type!=='email_template_created'||!d.version_id)return;const label=(d.name||('Modelo #'+d.version_id))+' · v'+(d.version_number||1)+' (Atual) · '+(d.subject||'');if(!templates.some(t=>String(t.id)===String(d.version_id)))templates.unshift({id:+d.version_id,name:d.name||'Novo e-mail',version_number:+d.version_number||1,subject:d.subject||'',is_current:1});const n=graph.nodes.find(x=>String(x.id)===String(d.node_id))||graph.nodes.find(x=>x.id===selected&&x.type==='email');if(!n||n.type!=='email')return;const c=n.config||(n.config={});c.templateVersionId=+d.version_id;c.templateLabel=label;if(c.abEnabled&&Array.isArray(c.variants)&&c.variants[0]){c.variants[0].templateVersionId=+d.version_id;c.variants[0].templateLabel=label}selected=n.id;render();inspect()});
