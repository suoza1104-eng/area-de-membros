...
<style>
.vv-chart-card canvas{height:280px!important;max-height:280px}.vv-status-card #vvStatus{height:190px!important;max-height:190px}.vv-status-card #vvStatusBars{height:180px!important;max-height:180px}.vv-overview-metrics{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.vv-rate.good strong{color:#86efac}.vv-rate.bad strong{color:#fca5a5}.vv-rate.warn strong{color:#facc15}.vv-funnel{display:grid;grid-template-columns:repeat(7,minmax(110px,1fr));gap:0;align-items:stretch;overflow:auto;padding-bottom:4px}.vv-funnel-step{position:relative;min-height:138px;padding:12px 10px 10px;border:1px solid var(--border);background:linear-gradient(180deg,rgba(250,204,21,.16),rgba(250,204,21,.06));clip-path:polygon(0 0,calc(100% - 18px) 0,100% 50%,calc(100% - 18px) 100%,0 100%,18px 50%)}.vv-funnel-step:first-child{clip-path:polygon(0 0,calc(100% - 18px) 0,100% 50%,calc(100% - 18px) 100%,0 100%)}.vv-funnel-step:last-child{clip-path:polygon(0 0,100% 0,100% 100%,0 100%,18px 50%)}.vv-funnel-step strong{display:block;font-size:22px}.vv-funnel-step small{display:block;color:var(--muted);font-size:10px;text-transform:uppercase}.vv-funnel-step span{display:block;margin-top:8px;color:#facc15;font-size:11px;font-weight:800}.vv-funnel-step em{display:block;margin-top:3px;color:#93c5fd;font-size:10px;font-style:normal}.vv-overview-wide{grid-column:1/-1}@media(max-width:1200px){.vv-funnel{grid-template-columns:repeat(4,minmax(130px,1fr))}}@media(max-width:700px){.vv-funnel{grid-template-columns:1fr}.vv-funnel-step,.vv-funnel-step:first-child,.vv-funnel-step:last-child{clip-path:none;border-radius:8px}}
</style>
<div class="vv">
  <div class="vv-head">
  ...
  <section class="vv-card vv-overview-wide">
    ...
    <div class="vv-funnel mt-3">
      <?php foreach($funnelStages as $s): ?>
        <div class="vv-funnel-step"><small><?=vv_h($s['label'])?></small><strong><?=(int)$s['count']?></strong><span><?=number_format((float)$s['pct_total'],1,',','.')?>% do total</span><em><?=number_format((float)$s['pct_prev'],1,',','.')?>% da etapa anterior</em></div>
      <?php endforeach; ?>
    </div>
  </section>
