  ...
  <section class="vv-card">
    ...
    <form method="post">
      ...
      <div class="vv-form-grid">
        ...
        <label class="vv-field"><label>Espaco fila automacao (seg.)</label><input type="number" name="automation_queue_spacing_seconds" value="<?=vv_h($cfg['automation_queue_spacing_seconds'] ?? 10)?>"><span class="vv-field-hint">Intervalo minimo entre novas tentativas de voz de automacoes.</span></label>
        <label class="vv-field"><label>Backoff limite simultaneo (seg.)</label><input type="number" name="automation_concurrency_backoff_step_seconds" value="<?=vv_h($cfg['automation_concurrency_backoff_step_seconds'] ?? 30)?>"><span class="vv-field-hint">Quanto acrescentar ao atraso quando a Telnyx/sistema estiver no limite.</span></label>
        <label class="vv-field"><label>Backoff maximo automacao (seg.)</label><input type="number" name="automation_concurrency_backoff_max_seconds" value="<?=vv_h($cfg['automation_concurrency_backoff_max_seconds'] ?? 1800)?>"><span class="vv-field-hint">Teto para refileirar por limite simultaneo antes de tentar novamente.</span></label>
