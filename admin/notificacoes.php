...
<div class="pn-preview-overlay" id="pnPreview" role="dialog" aria-modal="true" aria-labelledby="pnPreviewTitle">
    <div class="pn-preview-phone">
        <button class="pn-preview-close" id="pnPreviewClose" type="button" aria-label="Fechar">×</button>
...
(function(){
    ...
    document.querySelectorAll('[data-preview]').forEach(function(button){button.addEventListener('click',function(){const mode=modes[button.dataset.preview];title.textContent=mode.title;text.textContent=mode.text;cta.textContent=mode.cta;benefits.innerHTML='';mode.benefits.forEach(function(item){const span=document.createElement('span');span.textContent=item;benefits.appendChild(span)});overlay.classList.add('open');document.body.style.overflow='hidden'})});
