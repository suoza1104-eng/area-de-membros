...
const FP_RELAY_VERSION = '2026-08-04.1';
    ...
    $stmt = $pdo->prepare("
        INSERT INTO inbound_logs
            (request_uuid, method, content_type, content_length, remote_addr, user_agent, headers_json, query_json,
             body_raw, body_sha256, parsed_type, parsed_json, transaction_id, firepay_status, buyer_email, buyer_name,
             product_name, received_at)
        VALUES
            (:request_uuid, :method, :content_type, :content_length, :remote_addr, :user_agent, :headers_json, :query_json,
             :body_raw, :body_sha256, :parsed_type, :parsed_json, :transaction_id, :firepay_status, :buyer_email, :buyer_name,
             :product_name, :received_at)
...
function fp_dashboard(): void
{
    ...
    if ($date !== '') {
        $where[] = "date(i.received_at) = :date";
        $params[':date'] = $date;
    }
    if ($q !== '') {
        $where[] = "(i.transaction_id LIKE :q OR i.buyer_email LIKE :q OR i.buyer_name LIKE :q OR i.product_name LIKE :q)";
        $params[':q'] = '%' . $q . '%';
    }
    if ($status !== '') {
        $where[] = "LOWER(COALESCE(i.firepay_status,'')) = LOWER(:status)";
        $params[':status'] = $status;
    }
    if ($queueStatus !== '') {
        $where[] = "q.status = :queue_status";
        $params[':queue_status'] = $queueStatus;
    }
    $sqlWhere = $where ? 'WHERE ' . implode(' AND ', $where) : '';

    $stmt = $pdo->prepare("
        SELECT i.*, q.id AS queue_id, q.status AS queue_status, q.attempts, q.next_attempt_at, q.sent_at,
               q.last_http_status, q.last_error, q.last_response_body, q.target_url,
               (SELECT COUNT(*) FROM outbound_logs o WHERE o.queue_id=q.id) AS outbound_count
        FROM inbound_logs i
...
.filters{display:grid;grid-template-columns:150px 1fr 150px 150px auto;gap:8px;align-items:end}
...
<div class="wrap">
  ...
  <div class="panel">
    <form class="filters" method="get">
      ...
      <div><label>Busca</label><input name="q" value="<?= fp_html($q) ?>" placeholder="email, nome, transacao, produto"></div>
      <div><label>Status entrada</label><input name="status" value="<?= fp_html($status) ?>" placeholder="paid, waiting"></div>
  ...
  <div class="panel" style="overflow:auto">
    <table>
      <thead><tr><th>Hora</th><th>Entrada</th><th>Aluno/Produto</th><th>Fila</th><th>Saida</th><th>Detalhes</th></tr></thead>
      <tbody>
      <?php foreach ($rows as $row): ?>
        <tr>
          <td><strong><?= fp_html((string)$row['received_at']) ?></strong><div class="muted">#<?= (int)$row['id'] ?> / fila <?= fp_html((string)($row['queue_id'] ?? '-')) ?></div></td>
          <td>
            <div>Tx: <strong><?= fp_html((string)($row['transaction_id'] ?? '-')) ?></strong></div>
            <div><?= fp_status_badge((string)($row['firepay_status'] ?? '')) ?></div>
          ...
          <td>
            ...
            <div class="muted"><?= fp_html((string)($row['product_name'] ?? '-')) ?></div>
          </td>
          <td>
          ...
          <td>
            <div>HTTP: <strong><?= fp_html((string)($row['last_http_status'] ?? '-')) ?></strong></div>
            <div class="muted">enviado: <?= fp_html((string)($row['sent_at'] ?? '-')) ?></div>
            <?php if (!empty($row['last_error'])): ?><div style="color:#b91c1c"><?= fp_html((string)$row['last_error']) ?></div><?php endif; ?>
          </td>
          <td>
            <details><summary>Recebido</summary><pre><?= fp_html(json_encode(json_decode((string)$row['parsed_json'], true), JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?: (string)$row['parsed_json']) ?></pre></details>
            <details><summary>Enviado</summary><pre><?= fp_html((string)($row['last_response_body'] ?? '')) ?></pre><div class="muted"><?= fp_html((string)($row['target_url'] ?? '')) ?></div></details>
          </td>
        </tr>
      <?php endforeach; ?>
      <?php if (!$rows): ?><tr><td colspan="6">Nenhum registro encontrado.</td></tr><?php endif; ?>
      </tbody>
    </table>
  </div>
</div>
