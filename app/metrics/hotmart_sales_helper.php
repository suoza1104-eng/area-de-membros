...
function hotmart_find_matching_user(PDO $pdo, string $email, string $phone): ?array
{
    ...
    try {
        if ($email !== '') {
            $stmt = $pdo->prepare('SELECT id, email, telefone, nome FROM users WHERE LOWER(TRIM(email)) = :email LIMIT 1');
        ...
        if ($phone !== '') {
            $stmt = $pdo->prepare('SELECT id, email, telefone, nome FROM users WHERE telefone IS NOT NULL AND telefone <> "" AND (telefone = :phone OR RIGHT(telefone, 8) = RIGHT(:phone, 8)) LIMIT 1');
