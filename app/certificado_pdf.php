...
<div class="page">
    ...
    <?php foreach ($layout['front'] as $item):
        ...
        if ($fieldKey === 'qr'):
            ...
            $qrUrlEncoded = 'https://chart.googleapis.com/chart?chs=' . $qrSize . 'x' . $qrSize . '&cht=qr&choe=UTF-8&chl=' . urlencode($fieldValues['qr']);
    ?>
    <div class="field-qr" style="left:<?= $x ?>%;top:<?= $y ?>%;">
        <img src="<?= htmlspecialchars($qrUrlEncoded, ENT_QUOTES, 'UTF-8') ?>" width="<?= $qrSize ?>" height="<?= $qrSize ?>" alt="QR">
    </div>
    <?php else:
        $valor = $fieldValues[$fieldKey] ?? '';
        if ($valor === '') continue;
    ?>
    <div class="field" style="left:<?= $x ?>%;top:<?= $y ?>%;font-size:<?= $font ?>px;">
    ...
    <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;">
        <div style="text-align:center;width:80%;">
            ...
            <div style="font-size:16px;line-height:1.5;margin-bottom:30px;">
                concluiu com êxito o curso. Emitido em <?= htmlspecialchars($dataEmissao, ENT_QUOTES, 'UTF-8') ?>.
            </div>
