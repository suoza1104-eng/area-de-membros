...
$pdo = getPDO();
if ((string)get_setting('pagarme_enabled', '0') !== '1') {
    http_response_code(403);
    echo json_encode(['ok' => false, 'message' => 'Integracao Pagar.me pausada'], JSON_UNESCAPED_UNICODE);
    exit;
}
...
try {
    $result = pagarme_process_webhook(
        $pdo,
        $payload,
    ...
    app_log('Falha no webhook Pagar.me', ['error' => $e->getMessage()]);
    http_response_code(500);
    echo json_encode(['ok' => false, 'message' => 'Falha ao processar webhook Pagar.me'], JSON_UNESCAPED_UNICODE);
}
