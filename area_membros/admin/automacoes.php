...
function emailConfigHtml(c){c.variants=Array.isArray(c.variants)&&c.variants.length?c.variants:[{id:uid('var'),label:'A',weight:50,templateVersionId:c.templateVersionId||0,templateLabel:c.templateLabel||''},{id:uid('var'),label:'B',weight:50,templateVersionId:0,templateLabel:''}];return field('Modelo publicado',`<select id="iTemplate" ${c.abEnabled?'disabled':''}>${templateOptions(c.templateVersionId, c.templateLabel)}</select>`)+`<label class="afe-check"><input id="iAbEnabled" type="checkbox" ${c.abEnabled?'checked':''}> Ativar teste A/B/n neste e-mail</label>`+(c.abEnabled?`<div class="afe-config-box"><div class="afe-field"><label>Variantes A/B/n</label><div class="afe-copy">O peso define a proporção dos disparos. As métricas abaixo usam os envios deste fluxo para cada modelo.</div><div class="afe-fields">${c.variants.map(abVariantHtml).join('')}</div><button type="button" class="btn btn-ghost btn-xs" id="addVariant">+ Inserir variante</button></div></div>`:'')}
