...
/**
 * Normaliza URLs/IDs do YouTube para um src de iframe no formato https://www.youtube.com/embed/ID
 */
function youtube_embed_src(string $value): string {
    ...
    // Se for só o ID (sem http), já monta o embed direto
    if (strpos($value, 'http') !== 0 && strpos($value, '<') === false) {
        return 'https://www.youtube.com/embed/' . $value;
    }

    // Se já for um src de /embed/, usa direto
    if (stripos($value, 'youtube.com/embed') !== false) {
        return $value;
    }

    // Se for URL completa http(s)
    if (strpos($value, 'http') === 0) {
        ...
        // Links do tipo youtu.be/ID
        if (stripos($host, 'youtu.be') !== false) {
            $id = ltrim((string)$path, '/');
            return 'https://www.youtube.com/embed/' . $id;
        }

        // Links do tipo youtube.com/watch?v=ID
        if (stripos($host, 'youtube.com') !== false) {
            ...
            // Links já no formato /embed/ID
            if (!empty($path) && strpos($path, '/embed/') !== false) {
                $id = substr($path, strpos($path, '/embed/') + 7);
                return 'https://www.youtube.com/embed/' . $id;
            }
        }
    }

    // fallback: devolve a própria string
    return $value;
}
...
// Carrega configurações da aplicação (cores, título, etc.)
...
// Normaliza tipo html para embed
if ($videoType === 'html') {
    $videoType = 'embed';
}

// Detecta se o campo de vídeo já contém código embed/JS (iframe, script etc.)
$hasEmbedTags = false;
if ($videoUrl !== '') {
...
// ==== Aulas e progresso (para barra de progresso e carrossel) ====
$stLessons = $pdo->query("
    SELECT id, titulo, slug, ordem, thumb_url, conta_para_conclusao, ativo
    FROM lessons
    WHERE ativo = 1
...
// ====== Mapa de liberação BLINDADO (não depende de contagem, nem quebra com buracos na ordem) ======
// Regra: uma aula só fica liberada se TODAS as aulas anteriores (pela ordenação atual) estiverem concluídas.
$unlockMap = [];
$lessonById = [];
$allPrevDone = true;
$firstPendingId = null;
...
// Se o aluno tentar acessar uma aula bloqueada (por URL direta), redireciona para a primeira pendente (ou trilha)
...
// Regra de liberação:
// - Enquanto não concluir todas as obrigatórias: libera só até a próxima (max obrigatória concluída + 1)
// - Ao concluir 100% das obrigatórias (ou se não houver obrigatórias): libera todas as aulas
if ($totalObrigatorias <= 0 || $totalConcluidas >= $totalObrigatorias) {
    $ordemLiberadaMax = PHP_INT_MAX;
} else {
    $ordemLiberadaMax = $maxOrdemConcluida + 1;
    if ($ordemLiberadaMax < 1) {
        $ordemLiberadaMax = 1;
    }
}
$percent = 0;
...
try {
    ...
    ");
    if ($stRec) {
        $recommendedCourses = $stRec->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (Throwable $e) {
    $recommendedCourses = [];
}
